...
class PadImg(Pad):
    def __init__(self,
                 ...
                 pad_val=0,
                 apply_to='noisy_img'):
        super().__init__(size=size, size_divisor=size_divisor, pad_to_square=pad_to_square, pad_val=pad_val)
    ...
    def _pad_img(self, results):
        ...
        # for key in results.get(self.apply_to):
        for key in results.get('img_fields', ['noisy_img', ]):
        # for key in results.get('img_fields', [self.apply_to, ]):
            # print(key)
            if self.pad_to_square:
                max_size = max(results[key].shape[:2])
                self.size = (max_size, max_size)
...
class Normalize:
    ...
    def __init__(self, mean, std, apply_to='img', to_rgb=True):
