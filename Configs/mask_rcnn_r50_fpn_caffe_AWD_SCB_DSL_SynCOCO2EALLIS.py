...
test_lod_coco = dict(
    ...
    # ann_file='data/eallis/annotations/eallis_coco_JPG_traintest+1.json',
...
BATCHSIZE = 8
GPU = 1
# WARNING: `coco` and `test_lod_coco` use different class orders, so training on `coco` scrambles per-class AP; use `train_eallis` (same order as test) or reorder the `coco` classes.
data = dict(
...
log_level = 'INFO'
work_dir = './work_dir'
load_from = 'Checkpoints/Checkpoint1.pth'
# from mmdetection
